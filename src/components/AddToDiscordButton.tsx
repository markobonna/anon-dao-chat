...
export function AddToDiscordButton() {
  const handleAddToDiscord = () => {
    ...
    const authUrl = `https://discord.com/api/oauth2/authorize?client_id=${DISCORD_CLIENT_ID}&permissions=${DISCORD_PERMISSIONS}&scope=${DISCORD_SCOPE}`;
    window.open(authUrl, '_blank');
  };

  return (
    <Button onClick={handleAddToDiscord} className="bg-[#5865F2] hover:bg-[#4752C4] text-white">
      <svg
